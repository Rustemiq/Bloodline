...
class Player(pygame.sprite.Sprite):
    def __init__(self, weapon, pos_x, pos_y, *groups):
        ...
        self.rect = pygame.Rect(tile_size * pos_x, tile_size * pos_y, 45, 45)
    ...
    def turn_to_mouse(self, mouse_pos):
        ...
        self.image_offset = rotate_on_pivot(
            player_center, self.direction, self.image, self.sample_image
        )
    ...
    def is_trigger_touched(self):
        return (
            pygame.sprite.spritecollideany(self, self.trigger_tile_group)
            is not None
        )
    ...
    def die(self):
        if self.is_alive:
            ...
            self.image = pygame.transform.rotate(
                self.sample_image, self.direction
            )
            self.image_offset = rotate_on_pivot(
                dead_player_center,
                90 - self.direction,
    ...
    def update(self):
        if self.hit_animation.curr_frame > 0:
            if self.weapon == "empty" and self.is_alive:
                self.image = self.sample_image = self.hit_animation.animate()
                self.turn_to_mouse(pygame.mouse.get_pos())
            else:
