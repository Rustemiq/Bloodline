...
def convert_to_hand(weapon, bullets_group, all_sprites):
    if weapon.type == 'shotgun':
        weapon = ShotgunInHand(weapon.ammo)
        weapon.add_inter_groups(bullets_group, all_sprites)
    if weapon.type == 'uzi':
...
def convert_to_item(weapon, player_rect, weapons_group, all_sprites):
    if weapon.type != 'knife':
